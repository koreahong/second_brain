...
- 웹소켓(WebSocket): 실시간 양방향 통신을 제공하는 프로토콜로, HTTP와 달리 연결을 유지하면서 언제든 데이터를 주고 받을 수 있습니다.
  - 웹소켓(WebSocket)은 실시간 양방향 통신을 지원하는 통신 프로토콜 중 하나입니다. 기존의 HTTP 프로토콜은 클라이언트가 서버에게 요청을 보내고 서버가 응답을 보내는 단방향 통신이었습니다. 하지만 웹소켓은 한 번 연결이 되면 양쪽 모두가 자유롭게 데이터를 주고 받을 수 있는 양방향 통신을 제공합니다.
    웹소켓은 다음과 같은 특징을 가지고 있습니다:

    1. 실시간 양방향 통신: 웹소켓은 클라이언트와 서버 간에 양방향 통신이 가능하므로, 서버가 클라이언트에게 데이터를 푸시하거나 클라이언트가 서버에게 요청하지 않아도 서로간에 데이터를 주고 받을 수 있습니다.
    1. 낮은 지연과 오버헤드: HTTP와 비교하여 낮은 지연과 오버헤드 #overhead 를 가지며, 효율적인 데이터 전송이 가능합니다. 웹소켓은 계속해서 연결을 유지하고 있어, 새로운 데이터가 도착하면 즉각적으로 전송됩니다.
