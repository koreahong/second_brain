...
**시간적 맥락**: Claude Code 마스터를 위해 3개 아티클을 체계적으로 학습 → 정현과의 지식 교류 준비

- [[03-Resources/Technology/Claude-Code/Claude-Code-7가지-필수-플러그인|7가지 필수 플러그인]] (2분 전 읽음)
  - **순서의 의미**: Tricks를 먼저 익힌 후 → Plugins로 자동화 (학습 → 자동화 흐름)
  - **통합 활용**: Trick #10 (MCP Server Chains) + Plugin 마켓플레이스 = 최강 조합
  - **예시**: Trick #1 (Think Mode) 활성화 → Security-Scanner-Pro로 자동 검증

- [[03-Resources/Technology/Claude-Code/Claude-Code-Pixeltable-무한-메모리-엔진|Pixeltable 무한 메모리]] (5분 전 읽음)
  - **주제 연결**: Trick #2 (MCP Servers) 활용 → Pixeltable을 MCP로 통합
  - **Context 관리**: Trick #8 (Context-Aware Documentation) + Pixeltable 영속성 = 완벽한 문서화
  - **아키텍처**: Trick #11 (Low-level Design) 철학 → Pixeltable 커스터마이징에 적용

### 실무 적용 (크래프트테크놀로지스)
- [[30-Flow/Life-Insights/Personal/MCP-아키텍처와-데이터-거버넌스-최적화|MCP 아키텍처 최적화]] (2025-12-02)
  - **적용한 Tricks**:
    - Trick #2: dbt, Airflow, DataHub, Snowflake MCP 서버 체인 구축
    - Trick #10: 중앙 Agent가 전문 Agent orchestrate (MCP Server Chains 패턴)
    - Trick #8: 프로젝트별 프롬프트 최적화 (데이터 엔지니어링 특화)
  - **토큰 최적화**: Context-aware prompting으로 토큰 사용량 감소
  - **다음 적용**: Trick #9 (Intelligent Code Search)로 DBT 모델 검색 자동화

- [[03-Resources/Articles/Claude-Code-Context-관리-97퍼센트-실패하는-이유|Context 관리 실패 이유]] (2025-12-04)
  - **문제 해결**: Context Window Death Spiral → Trick #15 (Natural Language Architecture Planning)로 해결
  - **아키텍처 설계**: 4-Layer Orchestra 개념 → Trick #10 (MCP Chains) 실전 적용
  - **품질 향상**: Trick #4 (Defensive Coding) → 버그 사전 방지

- [[03-Resources/Articles/Claude-Code-개발-효율화-10가지-명령어|개발 효율화 10가지 명령어]] (2025-12-04)
  - **비교**: 15 Pro Tricks (사용 기법) vs 10 Commands (워크플로우 자동화)
  - **단계별 적용**: Tricks 숙달 → Custom Commands 작성 → 팀 전파
  - **우선순위**: Trick #1, #2, #8 먼저 익히기 (Think Mode, MCP, Context-Aware)

### 주간 회고 (같은 시기)
- [[02-Areas/크래프트테크놀로지스/Experience/Weekly/2025년-12월-05일|12월 5일 회고]] (1일 전)
  - **공유 내용**: 정현에게 Trick #1 (Think Mode), Trick #2 (MCP Servers) 설명
  - **실무 반응**: Windows 환경에서 MCP 설정 복잡 → Mac 환경 기준 가이드 한계
  - **깨달음**: "에이전트가 모든 사람에게 편하지 않다" → 접근성 개선 방안 필요
  - **다음 액션**: Windows 환경 MCP 설정 간소화 방법 연구
